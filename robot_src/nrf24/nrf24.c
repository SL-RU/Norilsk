...
void RF24_csn(RF24 *r, bool mode)
{
...
void RF24_ce(RF24 *r, bool level)
{
...
uint8_t RF24_read_register(RF24 *r, uint8_t reg, uint8_t* buf, uint8_t len)
{
    uint8_t status;
...
uint8_t RF24_write_register(RF24 *r, uint8_t reg, uint8_t value)
{
    uint8_t status;
    
    RF24_beginTransaction(r);
    if (reg < nRF24_CMD_W_REGISTER) {
        ...
        if ((reg != nRF24_CMD_FLUSH_TX) && (reg != nRF24_CMD_FLUSH_RX) && \
            ...
            // Send register value
            tx[1] = value;
            HAL_SPI_Transmit(r->spi, tx, 2, 10);
        }
        else {
            HAL_SPI_Transmit(r->spi, tx, 1, 10);
        }
    }
    RF24_endTransaction(r);
...
RF24_RF24(uint16_t _cepin, uint16_t _cspin):
...
RF24_RF24(uint16_t _cepin, uint16_t _cspin, uint32_t _spi_speed):
